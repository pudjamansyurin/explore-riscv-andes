...
int32_t i2cx_initialize(NDS_I2C_SignalEvent_t cb_event, I2C_RESOURCES* i2c) {
	...
	// enable CPU IRQ5(I2C)
	// PSW.CPL is interrupt priority level 0~3, 3 is the lowest, be able to be preemptive by another int
	__nds32__set_int_priority(NDS32_HWINT(IRQ_I2C_VECTOR), 3);
	__nds32__enable_int(NDS32_HWINT(IRQ_I2C_VECTOR));
	// enable PSW.GIE
	__nds32__setgie_en();
...
int32_t i2cx_uninitialize(I2C_RESOURCES* i2c) {
	...
	// disable CPU IRQ5(I2C)
	__nds32__disable_int(NDS32_HWINT(IRQ_I2C_VECTOR));
	// disable PSW.GIE //driver can't disable GIE, since other driver still use GIE
	// __nds32__setgie_dis();
	__nds32__isb();
...
int32_t i2cx_master_transmit(uint32_t addr, const uint8_t* data, uint32_t num, bool xfer_pending, I2C_RESOURCES* i2c) {
	...
	if (i2c->dma_tx) {
		...
		stat = dma_channel_configure(i2c->dma_tx->channel,
		                               (uint32_t)(&i2c->info->middleware_tx_buf[0]),
		                               (uint32_t)(&(i2c->reg->DATA)),
		                               num,
...
int32_t i2cx_master_receive(uint32_t addr, uint8_t* data, uint32_t num, bool xfer_pending, I2C_RESOURCES* i2c) {
	...
	if (i2c->dma_rx) {
		...
		stat = dma_channel_configure(i2c->dma_rx->channel,
		                               (uint32_t)(&(i2c->reg->DATA)),
		                               (uint32_t)(&i2c->info->middleware_rx_buf[0]),
		                               num,
		                               DMA_CH_CTRL_SBSIZE(DMA_BSIZE_1)               |
		                               DMA_CH_CTRL_SWIDTH(DMA_WIDTH_BYTE)            |
		                               DMA_CH_CTRL_DWIDTH(DMA_WIDTH_BYTE)            |
...
void i2cx_slave_addr_hit_handler(I2C_RESOURCES* i2c) {
	...
	if (i2c->info->Driver_State & I2C_DRV_SLAVE_RX) {
		...
		if (i2c->dma_rx) {
			...
			stat = dma_channel_configure(i2c->dma_rx->channel,
			                               (uint32_t)(&(i2c->reg->DATA)),
			                               (uint32_t)(&i2c->info->Xfer_Data_Rd_Buf[i2c->info->Xfered_Data_Rd_Ptr]),
			                               MAX_XFER_SZ,
			                               DMA_CH_CTRL_SBSIZE(DMA_BSIZE_1)               |
			                               DMA_CH_CTRL_SWIDTH(DMA_WIDTH_BYTE)            |
			                               DMA_CH_CTRL_DWIDTH(DMA_WIDTH_BYTE)            |
